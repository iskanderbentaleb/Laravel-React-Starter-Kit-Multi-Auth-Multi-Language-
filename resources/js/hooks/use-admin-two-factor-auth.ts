...
export const useAdminTwoFactorAuth = () => {
    ...
    const fetchQrCode = useCallback(async (): Promise<void> => {
        try {
            const { svg } = await fetchJson<{ svg: string }>('/admin/user/two-factor-qr-code');
            setQrCodeSvg(svg);
        } catch {
    ...
    const fetchSetupKey = useCallback(async (): Promise<void> => {
        try {
            const { secretKey: key } = await fetchJson<{ secretKey: string }>(
                '/admin/user/two-factor-secret-key',
            );
    ...
    const fetchRecoveryCodes = useCallback(async (): Promise<void> => {
        try {
            clearErrors();
            const codes = await fetchJson<string[]>('/admin/user/two-factor-recovery-codes');
